...
/// more packets are available when the socket is READABLE.
///
...
impl UdpQueue {
    ...
    /// possible UDP packet is around 64KB.  `tmp` may be a reference
    /// to an array on the stack, or you might wish to keep a
    /// temporary buffer permanently allocated somewhere if you need
